  ...
  return (
    <div className="min-h-screen text-slate-100 relative overflow-hidden font-sans">
      ...
      <div className="fixed inset-0 z-0">
        {/* L'image de fond avec effet de parallaxe fixe */}
        <div 
          className="absolute inset-0 bg-cover bg-center bg-no-repeat bg-fixed"
          style={{ backgroundImage: "url('/1781-2.jpg')" }}
        />
        
        {/* Overlay sombre pour la lisibilité (90% d'opacité) */}
        <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-[1px]" />

        {/* Ta grille technique originale */}
        <div className="absolute inset-0 grid-tech opacity-20" />
        
        {/* Dégradé radial pour masquer les bords de l'écran */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sectors.map((sector, index) => (
            <Card key={index} className="card-premium group border-white/5 bg-slate-900/40 backdrop-blur-md hover:border-primary-500/50 transition-all duration-500 p-6 flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start mb-4">
                  <div className="p-3 rounded-xl bg-slate-900/80 group-hover:scale-110 transition-transform duration-300 border border-white/5">
                    {sector.icon}
                  </div>
                ...
                <ul className="space-y-2 mb-6">
                  {sector.features.map((feat, i) => (
                    <li key={i} className="flex items-center text-xs text-slate-300">
                      <div className="w-1.5 h-1.5 rounded-full bg-primary-500 mr-2 shadow-[0_0_8px_rgba(var(--primary-rgb),0.6)]" />
                      {feat}
                    </li>
                  ))}
                </ul>
        ...
        <div className="flex flex-col items-center justify-center pt-16 space-y-6">
          ...
          <Link href="./#trial">
            <Button className="btn-neon px-10 py-6 text-lg font-bold uppercase tracking-tighter shadow-2xl">
