...
const filteredRisks = risks.filter(risk => {
  ...
  // Correction : On s'assure que user existe avant d'accéder à user.role
  const isSimpleUser = user?.role === UserRole.UTILISATEUR;
  const isCreator = risk.createdByUserId === user?.id;

  // Si c'est un UTILISATEUR, il doit être le créateur. Sinon (Admin/etc), il voit tout.
  const matchUser = !isSimpleUser || isCreator;

  return matchCategory && matchSeverity && matchUser;
});
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <h1 className="text-2xl font-bold">
           {user?.role === UserRole.UTILISATEUR 
