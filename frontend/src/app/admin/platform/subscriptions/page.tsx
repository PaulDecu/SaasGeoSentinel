...
export default function AdminSubscriptionsPage() {
  ...
  const [isTriggering, setIsTriggering] = useState(false); // État pour le chargement du bouton mail
  ...
  // Fonction pour déclencher manuellement les relances mail
  const handleTriggerNotifications = async () => {
    if (!confirm('Voulez-vous vraiment lancer manuellement le scan et l\'envoi des relances mail ?')) return;

    setIsTriggering(true);
    try {
      await subscriptionsApi.triggerNotifications();
      toast.success('Le traitement des notifications a été lancé avec succès.');
    } catch (error) {
      toast.error("Erreur lors du déclenchement : " + getErrorMessage(error));
    } finally {
  ...
  return (
    <AuthLayout requiredRoles={[UserRole.SUPERADMIN]}>
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* Header avec bouton de déclenchement manuel */}
